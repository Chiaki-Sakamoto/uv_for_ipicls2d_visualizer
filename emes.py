...
#
#  ipicls2d による計算結果で emes ディレクトリに保存される結果を2次元プロットするプログラム
#  プロットした結果は emes_plot に保存される．
#  emes file はレーザー1周期で平均した密度の値を
#  1 列目 イオン密度，2 列目 電子密度
#  で出力している．
#
# 使い方[test.py]
#   emes.py を PATH の通ったところに保存し
#    > chmod +x emes.py
#   で実行許可を与える．
#
#  計算のときに用いた namelist file を引数として与え，設定された値を読み込んでいる．
#     > emes.py [-l, -f -t] -n namelist_file
#
#   > emes.py -h または emes.py --help
#
...
# if ret.returncode==1:
#     print("ffmpeg はインストールされていません．emes.py の実行には ffmpeg が必要です．")
#     exit()
# ///////////////// parse /////////////////////////////////////
help_desc_msg = """【概要】
ipicls2d によるエネルギー密度の結果 emes を2次元カラーマップで png 形式で emes_plot 内
に保存し，動画や与えたコマンドラインも保存する．
-f/--vmin, -t/--vmax の指定でカラーバーの最大最小を指定
-x/--xmin, -X/--xmax, -y/--ymin, -Y/--ymax の指定でx軸とy軸のプロット範囲を指定
-s/--snap で整数番目のファイルのスナップを表示
-l/--line y=0 のx軸上の値のラインプロットを表示
"""

parser = argparse.ArgumentParser(
    prog="emes.py",
    usage="emes.py [-h] [-l --line] [-s --snap][-f --vmin] [-t --vmax] \
...
y_axis_title = "Energy"  # contorのz軸
out_file_suffix = "energy"  # png file の接頭語
particle_index = "energy_"  # 動画ファイルの接頭語
title = "Energy"  # グラフの上部中央のタイトル
...
#
# /////////////////////////////////////////////////////////////////////////////
def emes2D(datafile, DataNum, new_dir_path):
    Output = np.zeros(
        [grid_y, grid_x]
    ...
    cbar.set_label(y_axis_title + " [$\mathrm{J}$]", size=24)
...
# ax2.plot(X, Output2[y, :], c='w',  label='$E_x$', alpha=0.6)
...
#    if args.png:
#     plt.savefig(
#         new_dir_path + '/emes_Ey' + '{0:05d}'.format(DataNum) + '.png'
#     )  # emes_plot に png で保存
#     cmd5 = (
#         'ffmpeg -loglevel quiet -y -r 20 -i'
#         + './emes_plot/emes_Ey%5d.png -pix_fmt yuv420p'
#         + './emes_plot/output_Ey.mp4'
#         )
# # -y : 強制的に上書きする
#     cmd6 = 'open ./emes_plot/output_Ey.mp4'
#     subprocess.call(cmd5.split())  # ffmpeg でanimationを生成
#     print("x軸での変化を emes_plot 以下に output_Ey.mp4 として保存しました．")
#     # else:
#     #     plt.savefig(
#     #         new_dir_path + '/emes_Ey'+ '{0:05d}'.format(DataNum) +'.pdf'
#     #     )  # emes_plot に pdf で保存
#     plt.close()
...
#
# /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
def emes_line(datafile, DataNum, new_dir_path):
    Output = np.zeros(
        [grid_y, grid_x]
    ...
    # csv dir の作成と  csv ファイルの保存 (emes_line では csv file を作成している for gnuplot)
    cmd7 = "mkdir -p csv"
    ...
    plt.savefig(
        ...
        + ".png"
    )  # emes_plot に 散布図を n_e_iline_Num.png で保存
    plt.close()
...
def main():
    ...
    new_dir_path = FilePath + "emes_plot/"  # 保存先のファイルパス
    DataNum = 0  # 出力ファイルカウンターの初期化
    # path_namelist は利用していないので，コメントアウト
    # path_namelist = (
    ...
    if args.snap:
        ...
        snap_file = (
            FilePath + "emes/emes_all_" + "{0:05d}".format(snap_value) + ".gz"
        )  # snap の対象ファイル
        ...
        )  # 保存先のディレクトリ emes_plot を作成
        if args.line:  # -l or -line のとき，
            emes_line(snap_file, snap_value, new_dir_path)  # ラインプロットの関数の実行
            cmd5 = (
                "open ./emes_plot/"
                + out_file_suffix
                + "_line_"
    ...
    # emes ファイルの読み込み
    # emes dir の存在の有無も判定したほうがいい？  os.path.isdir()
    if (
        os.path.isfile(FilePath + "emes/emes_all_" + "{0:05d}".format(DataNum) + ".gz")
        is True
    ):
        # os.path.isfile フィアルの存在確認 True => 存在している
        while os.path.isfile(
            FilePath + "emes/emes_all_" + "{0:05d}".format(DataNum) + ".gz"
        ):
            # while で，*.gz な全部のファイルに対して実行する
            datafile = (
                FilePath + "emes/emes_all_" + "{0:05d}".format(DataNum) + ".gz"
            )  # データファイルのパス
            # datafile2 は利用していないので，コメントアウト
            # datafile2 = (
            #     FilePath
            #     + "empi/empi_all_"
            #     + "{0:05d}".format(DataNum)
            ...
            if args.line:  # -l or -line のとき，
                emes_line(datafile, DataNum, new_dir_path)  # ラインプロットの関数の実行
            # elif args.Ey:　　# -E のとき
    ...
    if args.line:  # -l or -line のとき，
        cmd3 = (
            "ffmpeg -loglevel quiet -y -r 20 -i ./emes_plot/"
            + out_file_suffix
            + "_line_%5d.png -pix_fmt yuv420p ./emes_plot/"
            + out_file_suffix
            + "_lineplot.mp4"
        )
        cmd4 = "open ./emes_plot/" + out_file_suffix + "_lineplot.mp4"
        subprocess.call(cmd3.split())  # ffmpeg でanimationを生成
        subprocess.call(cmd4.split())  # animation をopen
        print(
            "ラインプロットのアニメーションを emes_plot 以下に"
            + out_file_suffix
            + "_lineplot.mp4 として保存しました．"
        )
        f = open("./emes_plot/command_line.txt", "w")
        ...
        cmd1 = (
            "ffmpeg -loglevel quiet -y -r 20 -i ./emes_plot/"
            + out_file_suffix
            + "%5d.png -pix_fmt yuv420p ./emes_plot/"
            + out_file_suffix
            + "_animation.mp4"
        )
        cmd2 = "open ./emes_plot/" + out_file_suffix + "_animation.mp4"
        subprocess.call(cmd1.split())  # ffmpeg でanimation gif を生成
        subprocess.call(cmd2.split())  # animation gif をopen
        print(
            "アニメーションを emes_plot 以下に"
            + out_file_suffix
