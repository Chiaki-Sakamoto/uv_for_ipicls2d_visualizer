...
#  である．この3名に感謝．
#
#  ipicls2d による計算結果で emsi ディレクトリに保存される電磁場の結果を2次元プロットするプログラム
#  プロットした結果と動画およびコマンドラインで与えたオプションは emsi_plot に保存される．
#  emsi file は瞬時値の値であり，
#  1 列目 Bx，2 列目 By, 3列目 Ez
#  で出力している．
#
# 使い方[test.py]
#   emsi.py を PATH の通ったところに保存し
#    > chmod +x emsi.py
#   で実行許可を与える．
#
#  計算のときに用いた namelist file を引数として与え，設定された値を読み込んでいる．
#   > emsi.py [-h] [-l --line] [-s --snap] [-i --ex] [-j --ey] [-b --bz] \
#       [-t --vmax] [-x --xmin][-X --xmax] [-y --ymin] [-Y --ymax] \
#           [-n --namelist] namelist_file
#
#   > emsi.py -h または emsi.py --help
#
#  で簡単な使い方の help が表示される．
#
#  version 0.1 (20241114)
#  emps.py から emsi.py を作成した
#  version 0.2 (20250207)
#  -f/--vmin, -t/--vmax option を作成．負の数を指定するときはダブルコーテーションで数値をくくる
#  version 0.21
#  アニメの出力を整理した．main の最後の部分を修正．
#  -s / --snap option の設定を行った．
#  version 0.22
#  出力するアニメーションファイルを Bx_animation.mp4, By_animation.mp4, Ez_animation.mp4のように
#  物理量を明示するようにした．
#
#  todo:
...
# if ret.returncode==1:
#     print("ffmpeg はインストールされていません．emsi.py の実行には ffmpeg が必要です．")
#     exit()
# ////////////////////////////// parse ////////////////////////////////////////

help_desc_msg = """【概要】
ipicls2d によって計算された電磁場(Bx, By, Ez)を 2 次元カラーマップで emsi_plot 内に保存し，そのアニメーションも作成する．
-i/--bx で Bx, -j/--by で By，-b/--ez で Ez が得られる．
...
parser = argparse.ArgumentParser(
    prog="emsi.py",
    usage="emsi.py [-h] [-s --snap] [-l --line] [-i --ex, -j --ey, -k --bz] \
...
parser.add_argument(
    "-fft",
    "--fft",
    help="各周波数における y=0 での周波数分布をプロットする. ",
    action="store_true",
)
# parser.add_argument(
#    '-p', '--png',
...
#    action='store_true'
#    )
parser.add_argument("-i", "--bx", help="Bx を出力", action="store_true")
parser.add_argument("-j", "--by", help="By を出力", action="store_true")
parser.add_argument("-k", "--ez", help="Ez を出力", action="store_true")
...
parser.add_argument(
    "-fx", "--fxmin", help="fft時のプロットする範囲(x軸の最小値", action="store"
)
parser.add_argument(
    "-fX", "--fxmax", help="fft時のプロットする範囲(x軸の最大値", action="store"
)
parser.add_argument(
...
parser.add_argument(
    "-o",
    "--output",
    help="outputするファイル名. 主に -input オプションとの併用で用いる. ",
    action="store",
)
...
# initial : 初期印加電磁場(emsiの場合はEzextのみ．Bx, By には初期磁場は設定できない)
...
if args.ez:  # 3 行目のデータ
    ...
    initial = Ezext
    mag = 1
# m_e * speed_of_light * speed_of_light * 2 * pi
# / e_charge /(wavelength * 1e-6)
if particle == 0:
    print("Bx の等高線図を作成します．")
if particle == 1:
    print("By の等高線図を作成します．")
if particle == 2:
    print("Ez のマップを作成します．")
...
# ////////////////////// カラーマップを作成する関数 ////////////////////////////////
def emsi2D(datafile, DataNum, new_dir_path):
    Output = np.zeros([grid_y, grid_x])
    Data = np.loadtxt(
        datafile
    )  # データを読み込む．loadtxt では 対象ファイルが gz の場合はまず解凍する．
    for i in range(grid_y):
        for j in range(grid_x):
            Output[i, j] = Data[i * grid_x + j, particle] * mag
    ...
    if boost_opt is False:  # ブーストオプションを用いていない場合
        X = (
            np.linspace(
                -Output.shape[1] / 2, Output.shape[1] / 2 + 1, Output.shape[1] + 1
            )
            * (wavelength / ndav)
            * Nx_d
        )  # 中央を 0 にする
        Y = (
            np.linspace(
                -Output.shape[0] / 2, Output.shape[0] / 2 + 1, Output.shape[0] + 1
            )
            * (wavelength / ndav)
            * Ny_d
        )  # 中央を 0 にする
    ...
    if args.vmax:
        max_value = float(
            args.vmax
        )  # 文字列からfloat で浮動小数に変換する．color bar の最大値
    ...
    # pclorfast で，高さ方向の値で vmin=, vmax= で値を与えると，それらを 最小値，最大値として高さ方向(色の濃淡)の図を作成する．
    ...
    if particle == 0:
        cbar.set_label("Bx [norm.]", size=24)
    if particle == 1:
        cbar.set_label("By [norm.]", size=24)
    if particle == 2:
        cbar.set_label("Ez [norm.]", size=24)
    ...
    plt.tight_layout()
    plt.savefig(
        new_dir_path + "/" + title + "_emsi_" + "{0:05d}".format(DataNum) + ".png"
    )  # emsi_plot に png で保存
    plt.close("all")
...
#
# //////////////////////////////////////////////////////////////////////////////
def emsi_line(datafile, DataNum, new_dir_path):
    Output = np.zeros(
        [grid_y, grid_x]
    ...
    for i in range(grid_y):
        for j in range(grid_x):
            Output[i, j] = Data[i * grid_x + j, particle] + initial
            # ここ 2 は間違いではないか? particle に修正した
    # outmax 使ってないのでコメントアウトします
    # if args.vmax:
    ...
    if boost_opt is False:  # ブーストオプションを用いていない場合
        X = (
            np.linspace(-Output.shape[1] / 2, Output.shape[1] / 2 + 1, Output.shape[1])
            * (wavelength / ndav)
            * Nx_d
        )  # 中央を 0 にする
    else:
    ...
    y = grid_y // 2  # y 軸の中央
    ax.plot(X, Output[y, :])  # y 軸の中央をプロット
    ...
    with open(
        "csv/" + title + "_emsi_" + "{0:05d}".format(DataNum) + ".csv",
        mode="w",
        newline="",
    ) as file:
    ...
    # print(
    #     "Data for lineplot is saved as  csv/emsi_"
    ...
    f.canvas.draw()
    #  グラフの保存
    if args.input_file and args.output:
        plt.savefig(new_dir_path + "/" + args.output + ".png")
    else:
        plt.savefig(
            new_dir_path
            + "/"
...
def fft(datafile, DataNum, new_dir_path):
    Output = np.zeros([grid_y, grid_x])
    Data = np.loadtxt(datafile)

    for i in range(grid_y):
        for j in range(grid_x):
            Output[i, j] = Data[i * grid_x + j, particle]
    plt.rcParams["font.size"] = 20

    _, ax = plt.subplots(figsize=(10, 6))

    if boost_opt is False:
        X = (
            np.linspace(0, Output.shape[1] + 1, Output.shape[1])
            * (wavelength / ndav)
            * Nx_d
            / speed_of_light
        )
    else:
        if DataNum * int_snap <= nst_boost:
            X = (
                np.linspace(-NV, Output.shape[1] - NV, Out.shape[1])
                * (wavelength / ndav)
                * Nx_d
                / speed_of_light
            )
    ...
    centerGridY = grid_y // 2
    if args.xmin and args.xmax:
        x_mask = (X >= (float(args.xmin) + 160) / speed_of_light) & (
            X <= (float(args.xmax) + 160) / speed_of_light
        )
        x_cut = X[x_mask]
        centerData = Output[centerGridY, x_mask]
        print(x_cut * speed_of_light)
        print(len(x_cut))
        freq = np.fft.fftfreq(len(x_cut), d=(x_cut[1] - x_cut[0]) * 1e-6)
    else:
        centerData = Output[centerGridY, :]
        print(len(X))
        freq = np.fft.fftfreq(len(X), d=(X[1] - X[0]) * 1e-6)
    fftData = np.fft.fft(centerData)
    mask = freq >= 0
    freqMask = freq[mask]
    fftMaskData = fftData[mask]

    ax.plot(freqMask, np.abs(fftMaskData), color="black")
    cmd7 = "mkdir -p csv"
    subprocess.call(cmd7.split())
    with open(
        "csv/" + title + "_emsi_" + "{0:05d}".format(DataNum) + ".csv",
        mode="w",
        newline="",
        ...
        writer = csv.writer(file)
        writer.writerows(zip(X, Output[centerGridY, :]))
    if args.fxmin and args.fxmax:
        ax.set_xlim(float(args.fxmin), float(args.fxmax))
    ax.xaxis.set_major_formatter(FuncFormatter(lambda val, pos: f"{val / 1e12:.0f}"))
    ...
    if args.ymax:
        ...
        Y_ma = np.amax(Output[centerGridY, :]) + np.amax(Output[centerGridY, :]) * 0.1
        Y_mi = np.amin(Output[centerGridY, :]) - np.amin(Output[centerGridY, :]) * 0.1
    ax.set_ylim(Y_mi, Y_ma)
    ax.set_ylabel("Power Spectrum [V/m]")
    ax.tick_params(axis="x", pad=10)
    ax.tick_params(axis="y", pad=10)
    plt.tight_layout()
    ax.set_title("file=" + format(DataNum), size="small", loc="right")
    ...
    ax.grid(color="k", linestyle="--", linewidth=0.5)
    plt.savefig(
        new_dir_path + "/" + title + "_emsi_fft_" + "{0:05d}".format(DataNum) + ".png",
        transparent=True,
    )
    plt.close()
...
def main() -> None:
    ...
    # path_namelist は使用していないのでコメントアウトします
    # path_namelist = FilePath + args.namelist
    # args.namelist は，コマンドラインで与えた namelist file 名
    print("与えられた namelist からパラメーターを読み込みました．\n")

    if args.input_file:
        print(args.input_file)
        new_dir_path = "./test_plot"
        os.makedirs(name="./test_plot", exist_ok=True)
        if args.line:  # -l or -line のとき，
            emsi_line(
                args.input_file, DataNum, new_dir_path
            )  # ラインプロットの関数の実行
        elif args.fft:
            fft(args.input_file, DataNum, new_dir_path)
        else:
            emsi2D(args.input_file, DataNum, new_dir_path)
        return
    if args.snap:
        FilePath = "./"
        snap_value = int(args.snap)  # color bar の最小値
        snap_file = FilePath + "emsi/emsi_all_" + "{0:05d}".format(snap_value) + ".gz"
        print("指定時刻", snap_value)
        print("指定ファイル", snap_file)
        new_dir_path = FilePath + "emsi_plot"  # 保存先のファイルパス
        os.makedirs(
            new_dir_path, exist_ok=True
        )  # 保存先のディレクトリ emsi_plot を作成
        if args.line:  # -l or -line のとき，
            emsi_line(snap_file, snap_value, new_dir_path)  # ラインプロットの関数の実行
            cmd5 = (
                "open ./emsi_plot/"
                + title
                + "_emsiline_"
            ...
            emsi2D(snap_file, snap_value, new_dir_path)
            # カラーマップ作成関数 emsi2D を実行
            cmd6 = (
                "open ./emsi_plot/"
                + title
                + "_emsi_"
    ...
    # emsi ファイルの読み込み
    # emsi dir の存在の有無も判定したほうがいい？  os.path.isdir()
    if (
        os.path.isfile(FilePath + "emsi/emsi_all_" + "{0:05d}".format(DataNum) + ".gz")
        is True
    ):
        # os.path.isfile フィアルの存在確認 True => 存在している
        while os.path.isfile(
            FilePath + "emsi/emsi_all_" + "{0:05d}".format(DataNum) + ".gz"
        ):  # このwhile で各データをプロット．
            ...
            if args.line:  # -l or -line のとき，
                emsi_line(datafile, DataNum, new_dir_path)  # ラインプロットの関数の実行
            # elif args.Ey:  # -E のとき
            #     laser_E(datafile, datafile2, DataNum, new_dir_path)
            # # レーザー波形もプロット
            elif args.fft:
                fft(datafile, DataNum, new_dir_path)
            else:  # -l or --line でないとき，
                emsi2D(datafile, DataNum, new_dir_path)
                # カラーマップ作成関数 emsi2D を実行
            DataNum += 1  # ファイル時間をカウントアップ

    if args.line:  # -l or -line のとき，
        cmd3 = (
            "ffmpeg -loglevel quiet -y -r 20 -i ./emsi_plot/"
            + title
            + "_emsiline_%5d.png -pix_fmt yuv420p ./emsi_plot/"
            + title
            + "_lineplot.mp4"
        )
        cmd4 = "open ./emsi_plot/" + title + "_lineplot.mp4"
        subprocess.call(cmd3.split())  # ffmpeg でanimationを生成
        subprocess.call(cmd4.split())  # animation をopen
        print(
            "ラインプロットのアニメーションを emsi_plot 以下に"
            + title
            + "_lineplot.mp4 として保存しました．"
        )
        f = open("./emsi_plot/command_line.txt", "w")
        ...
        cmd3 = (
            "ffmpeg -loglevel quiet -y -r 20 -i ./emsi_plot/"
            + title
            + "_emsi_fft_%5d.png -pix_fmt yuv420p ./emsi_plot/"
            + title
            + "_fft_plot.mp4"
        )
        ...
        cmd1 = (
            "ffmpeg -loglevel quiet -y -r 20 -i ./emsi_plot/"
            + title
            + "_emsi_%5d.png -pix_fmt yuv420p ./emsi_plot/"
            + title
            + "_animation.mp4"
        )
        cmd2 = "open ./emsi_plot/" + title + "_animation.mp4"
        subprocess.call(cmd1.split())  # ffmpeg でanimation gif を生成
        subprocess.call(cmd2.split())  # animation gif をopen
        print(
            "アニメーションを emsi_plot 以下に"
            + title
        ...
        f.close()
    # else:
    #     print('処理するデータファイルが emsi ディレクトリにありません')
